...
// Replicating generateSeedFinanceData
function generateSeedFinanceData() {
  const transactions = [];
  
  const createTx = (date, type, category, amount, desc, method = 'Cash', term = 'Term 1') => {
    return {
...
const dbPath = path.join(process.cwd(), 'school_database.json');
try {
  const data = JSON.parse(fs.readFileSync(dbPath, 'utf8'));
  if (!data.finances) {
    data.finances = [];
  }
  const seedData = generateSeedFinanceData();
  data.finances.push(...seedData);
  fs.writeFileSync(dbPath, JSON.stringify(data, null, 2));
  console.log(`Successfully seeded ${seedData.length} finance transactions.`);
} catch (e) {
